...
impl SimState {
    ...
    pub fn add_flight(&mut self, flight: Flight, already_in_db: bool) -> Result<(), SimError> {
        {
    ...
    pub fn list_flights(&self) {
        ...
        thread::spawn(move || {
            ...
            loop {
                buffer.clear();
                if stdin().read_line(&mut buffer).is_ok() && buffer.trim() == "q" {
                    tx.send(true).unwrap();
                    break;
                }
