...
impl SimState {
    ...
    fn display_flights(&self) {
        ...
        let flights = self.flights.read().map_err(|_| SimError::Other("LockError".to_string()));
...

        {
            ...
            if let Ok(mut flight_lock) = flight.write() {
                ...
                if let Some(mut flight_states) = flight_states_lock {
                    flight_states.insert(flight_lock.flight_number.clone(), flight_lock.status.clone());
                }
            }
        }
