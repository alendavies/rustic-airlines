...
pub fn start(
    ...
    let self_ip;
    {
        ...
        if !is_seed {
            ...
            if let Ok(stream) = connect(seed_ip, INTERNODE_PORT, Arc::clone(&connections)) {
                ...
                let message = InternodeProtocolHandler::create_protocol_message(&node_guard.get_ip_string(), "HANDSHAKE", "_", true);
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                ...
                let mut stream = Arc::new(Mutex::new(stream)); // Encapsulamos el stream en Arc<Mutex<TcpStream>>
                let connections_clone = Arc::clone(&connections);

                thread::spawn(move || {
                    if let Err(e) = Node::handle_incoming_messages(node_clone, &mut stream, connections_clone, is_seed) {
    ...
    pub fn handle_incoming_messages(
        ...
        let internode_protocol_handler = InternodeProtocolHandler::new(node.clone());
        ...
        loop {    
            ...
            Self::execute_querys(node.clone(), connections.clone())?;
    ...
    fn execute_querys(node: Arc<Mutex<Node>>, connections: Arc<Mutex<HashMap<String, Arc<Mutex<TcpStream>>>>>) -> Result<(), NodeError> {
        ...
        for query_str in queries {
            ...
            let query = QueryCoordinator::new()
                ...
                let result = QueryExecution::new(node.clone(), connections.clone()).execute(query, false);
                ...
                match result {
                    ...
                    Err(e) => {println!("{:?} in {:?}", e, query_str)}, // Retorna el error si ocurrió uno
                }
        }
        Ok(())
    }
