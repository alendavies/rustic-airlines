...
fn main() -> Result<(), String> {
    ...
    // Esperar medio segundo antes de continuar
    thread::sleep(Duration::from_millis(500));

    // Parsear la IP del nodo
    let node_ip = Ipv4Addr::from_str(&args[1]).map_err(|_| "IP no válida".to_string())?;
    let seed_ip =
        Ipv4Addr::from_str(&"127.0.0.1".to_string()).map_err(|_| "IP no válida".to_string())?;

    // Crear el nodo con la dirección y un vector de seeds
    let node = Arc::new(Mutex::new(
        Node::new(node_ip, vec![seed_ip]).map_err(|e| e.to_string())?,
    ));
