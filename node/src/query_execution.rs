...
impl QueryExecution {
    ...
    pub fn new(node_that_execute: Arc<Mutex<Node>>, connections: Arc<Mutex<HashMap<String,  Arc<Mutex<TcpStream>>>>>) -> QueryExecution {
        QueryExecution {
            node_that_execute,
            connections,
        }
    ...
    pub fn execute_update(&self, update_query: Update, internode: bool, open_query_id: i32) -> Result<(), NodeError> {
       ...
       let table;
       {
        ...
        let where_clause = update_query.clone().where_clause.ok_or(NodeError::OtherError)?;
        where_clause.validate_cql_conditions(&primary_key, "")?;
        
        let value_to_hash = where_clause.get_value_primary_condition(&primary_key)?.ok_or(NodeError::OtherError)?;
        ...
        if self.update_in_this_node(update_query, table, &file_path, &temp_file_path).is_err() {
    ...
    fn update_in_this_node(&self, update_query: Update, table: Table, file_path: &str, temp_file_path: &str) -> Result<(), NodeError> {
        ...
        for line in reader.lines() {
            let line = line?;
            found_match |= self.update_or_write_line(&table, &update_query, &line, &mut temp_file)?;
        }
    ...
    /// Escribe el encabezado en el archivo temporal.
    fn write_header<R: BufRead>(&self, reader: &mut R, temp_file: &mut File) -> Result<(), NodeError> {
    ...
    fn update_or_write_line(
        ...
        if let Some(where_clause) = &update_query.where_clause {
            if where_clause.condition.execute(&column_value_map).unwrap_or(false) {
                found_match = true;
        ...
        writeln!(temp_file, "{}", columns.join(",")).map_err(|e| 
            NodeError::from(e)
        )?;
        Ok(found_match)
    }
    ...
    fn create_column_value_map(&self, table: &Table, columns: &[String], only_primary_key: bool) -> HashMap<String, String> {
    ...
    /// Agrega una nueva fila si no se encontró coincidencia.
    fn add_new_row(&self, table: &Table, update_query: &Update, temp_file: &mut File) -> Result<(), NodeError> {
        let mut new_row: Vec<String> = vec!["".to_string(); table.get_columns().len()];
        let primary_key = table.get_primary_key()?;
        let primary_key_index = table.get_column_index(&primary_key).ok_or(NodeError::OtherError)?;
    ...
    fn replace_original_file(&self, temp_file_path: &str, file_path: &str) -> Result<(), NodeError> {
    ...
    pub fn execute_delete(&self, delete_query: Delete, internode: bool, open_query_id: i32) -> Result<(), NodeError> {
        let table;
        {
            ...
            table = node.get_table(table_name.clone())?;
            let primary_key = table.get_primary_key()?;
            let where_clause = delete_query.clone().where_clause.ok_or(NodeError::OtherError)?;
            where_clause.validate_cql_conditions(&primary_key, "")?;
        ...
        if self.delete_in_this_node(delete_query, table, &file_path, &temp_file_path).is_err() {
            let _ = std::fs::remove_file(temp_file_path);
            return Err(NodeError::OtherError);
        }
        Ok(())
    ...
    fn delete_in_this_node(&self, delete_query: Delete, table: Table, file_path: &str, temp_file_path: &str) -> Result<(), NodeError> {
        // Abre los archivos
        let file = OpenOptions::new().read(true).open(&file_path)?;
        let mut reader = BufReader::new(file);
        let mut temp_file = self.create_temp_file(&temp_file_path)?;
    ...
    fn should_delete_line(&self, table: &Table, delete_query: &Delete, line: &str) -> Result<bool, NodeError> {
        let columns: Vec<String> = line.split(',').map(|s| s.trim().to_string()).collect();
        let column_value_map = self.create_column_value_map(table, &columns, true);
    
        if let Some(where_clause) = &delete_query.where_clause {
            return Ok(where_clause.condition.execute(&column_value_map).unwrap_or(false));
        }
    ...
    pub fn execute_select(&self, select_query: Select, internode: bool, open_query_id: i32) -> Result<Vec<String>, NodeError> {
        ...
        let where_clause = select_query.where_clause.clone().ok_or(NodeError::OtherError)?;
        where_clause.validate_cql_conditions(&primary_key, "")?;
    ...
    fn execute_select_in_this_node(&self, select_query: Select, table: Table) -> Result<Vec<String>, NodeError> {
    ...
    fn line_matches_where_clause(&self, line: &str, table: &Table, select_query: &Select) -> Result<bool, NodeError> {
        ...
        if let Some(where_clause) = &select_query.where_clause {
            Ok(where_clause.condition.execute(&column_value_map).unwrap_or(false))
        } else {
